...
- **UX e Tratamento de Erros**: Implementados estados de carregamento (Skeletons) no nível do App Router (`loading.tsx`), e *empty states* consistentes e ilustrados para Tabelas, Gráficos e Categorias. Validação de de dados com Zod. Server Actions centralizadas em blocos `try/catch` para devolver mensagens padronizadas de sucesso/falha, ativando `toasts` interativos (via biblioteca `sonner`) no lado do cliente.
